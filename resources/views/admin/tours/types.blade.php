...
<div class="flex items-center justify-between mb-6">
    <div>
        <h1 class="text-2xl font-extrabold text-slate-900 tracking-tight">Tour Types</h1>
        <p class="text-xs text-slate-500 font-medium mt-0.5">Manage tour classifications (Domestic, International, Corporate, Family, Adventure).</p>
    </div>
...
<div class="grid grid-cols-1 md:grid-cols-3 gap-6">
    <div class="bg-white p-6 rounded-2xl border border-slate-200/80 shadow-sm h-fit">
        <h3 class="font-extrabold text-slate-900 text-sm mb-4">Add New Tour Type</h3>
        <form action="{{ route('admin.tours.types.store') }}" method="POST" class="space-y-4">
            @csrf
            <div>
                <label class="block text-xs font-bold uppercase text-slate-700 mb-1">Type Name</label>
                <input type="text" name="name" required placeholder="e.g. International Luxury" class="w-full px-3.5 py-2 text-xs rounded-xl border border-slate-300 focus:ring-2 focus:ring-brand-500">
            </div>
            <div>
            ...
            <tbody class="divide-y divide-slate-100">
                @forelse($types as $type)
                    <tr>
                        ...
                        <td class="px-4 py-3 text-slate-500 font-mono">{{ $type->slug }}</td>
                        <td class="px-4 py-3 font-bold text-brand-600">{{ $type->tour_packages_count }} tours</td>
                    </tr>
                @empty
                    <tr>
                        <td colspan="3" class="text-center py-6 text-slate-400">No tour types created yet.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
